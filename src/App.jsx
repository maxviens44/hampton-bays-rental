...
export default function AvailabilityCalendar({ months = 12 }) {
    ...
    (async () => {
      try {
        ...
        if (priceRes.ok) {
          ...
          if (!cancelled) { setCurrency(cur); setDefaultPrice(def); setOverrides(map); }
        }
      } catch {
        if (!cancelled) { setBooked(new Set()); setCurrency("USD"); setDefaultPrice(1000); setOverrides({}); }
      } finally {
